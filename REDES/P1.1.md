...
#### ARP
EL protocol ARP **(Address Resolution Protocol)** para IPv4 permite la resolución de direcciones MAC. Se encarga la máquina que quiere solicitar una dirección MAC de preguntar a través de broadcast a quien le pertenece la dirección IP que la maquina tiene asociada. Por lo que, debe existir una dirección MAC e IP destino que en nuestro caso son primero broadcast ambas; y la del remitente en nuestro caso la Maquina  2 que tiene como dirección IP 192.168.0.2, como dirección MAC 08:00:27:92:d9:1b.
Si no existe error la máquina con la dirección IP asociada, en
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjU4OTIyNzk5LDEwNDk2OTU2MjcsLTE1MT
