...
#### ARP
EL protocol ARP **(Address Resolution Protocol)** para IPv4 permite la resolución de direcciones MAC. Se encarga la máquina que quiere solicitar una dirección MAC de preguntar a través de broadcast a quien le pertenece la dirección IP que la maquina tiene asociada. Por lo que, debe existir una dirección MAC e IP destino que en nuestro caso son primero broadcast ambas; y la del remitente en nuestro caso la Máquina  2 que tiene como dirección IP 192.168.0.2, como dirección MAC 08:00:27:92:d9:1b.
Si no existe error la máquina con la dirección IP asociada, en nuestro caso la Máquina 1, con dirección IP 192.168.0.1 que contesta con su dirección MAC 08:00:27:4c:af:03.
...
En el caso de ejecutar otra vez ping al principio de la ejecución no se utiliza el protocolo ARP para averiguar de quien es la dirección MAC asociada a la dirección IP 192.168.0.1. Sólo se repite la pregunta cada tanto para comprobar si esta siguen asociadas.
...
#### VM3 192.168.0.3/24 (eth0) 172.16.0.3/24 (eth1)
```bash
ip address add 192.168.0.3/24 dev eth0
ip link set dev eth0 up
ip address add 172.16.0.3/24 dev eth1
ip link set dev eth0 up
```

#### VM4
```bash
ip address add 172.16.0.3/24 dev eth0
ip link set dev eth0 up
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ4NzE5MDM2Niw4MjA1NzE4MzEsMTk4NT
E5NjMwNCwxMDQ5Njk1NjI3LC0xNTE4MTUxMDc1LDEzNTI0OTkw
NjcsMTc5OTQ4MDIwNSwxOTUzODQ1MjU3LDEzNzU1NTYxN119
-->